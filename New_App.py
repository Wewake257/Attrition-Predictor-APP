...
if not st.session_state.logged_in:
    ...
    with col2:
        ...
        <div class="login-card">
            <div class="login-title">OrgaKnow Secure Access</div>
            <div class="login-subtitle">
                Role-based access to Retention Intelligence
            </div>
        </div>
        ...
        if st.button("Login", use_container_width=True):
            ...
            if user:
                ...
                st.session_state.login_time_str = time.strftime(
                ...
                st.error("❌ Invalid credentials. Please contact system administrator.")
...
st.sidebar.caption(
    f"🕒 Last login: {st.session_state.get('login_time_str', 'N/A')}"
)

st.sidebar.caption(
    f"⏳ Session expires in: {remaining_minutes} minutes"
)
...
with tab2:
    ...
    if employee_df_full.empty:
        ...
        critical_roles = employee_df_full[
            employee_df_full["Role"].isin(["Executive", "Manager"])
        ]
        ...
        with k2:
            kpi_card("High Risk Employees", high_cnt, tone="high")

        with k3:
            kpi_card("Medium Risk Employees", med_cnt, tone="medium")

        with k4:
            kpi_card("Low Risk Employees", low_cnt, tone="low")
        ...
        treemap_df = employee_df_full.groupby(
            ["Department", "RiskBand"]
...
with tab4:
    ...
    if employee_df_full.empty:
        ...
        # ---------------------------------------
        high_risk_emps = employee_df_full[
            employee_df_full["RiskBand"] == "High"
        ...
        # SEGMENT STATS
        # ---------------------------------------
        segment_stats = employee_df_full.groupby(
            "Department"
        ).agg(
